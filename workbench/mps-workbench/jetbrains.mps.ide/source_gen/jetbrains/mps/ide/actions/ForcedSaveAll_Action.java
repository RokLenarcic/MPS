    ...
    ProgressManager.getInstance().runProcessWithProgressSynchronously(new Runnable() {
      public void run() {
        ...
        repo.getModelAccess().runReadAction(new Runnable() {
          public void run() {
            moduleRefs.value = Sequence.fromIterable((Iterable<SModule>) ((MPSProject) MapSequence.fromMap(_params).get("mpsProject")).getProjectModulesWithGenerators()).ofType(
                AbstractModule2.class).select(new ISelector<AbstractModule2, SModuleReference>() {
              public SModuleReference select(AbstractModule2 it) {
        ...
        for (final SModuleReference moduleRef : ListSequence.fromList(moduleRefs.value)) {
          ...
          repo.getModelAccess().runReadAction(new Runnable() {
            public void run() {
              AbstractModule2 module = (AbstractModule2) moduleRef.resolve(repo);
              if (module == null) {
                return;
              }
